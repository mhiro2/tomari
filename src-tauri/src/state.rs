...
pub struct AppState {
    ...
    /// Serializes whole-config mutations. A config import replaces every table
    /// and then rebuilds the live engines/shortcuts from the database; an
    /// interactive save/delete running in between would leave the in-memory
    /// state out of sync with disk. Every save/delete command and the import
    /// hold this for their whole operation, so they never interleave. It guards
    /// the *sequence* of operations, not a value, hence `Mutex<()>`.
...
impl AppState {
    ...
    /// Acquire the config-mutation lock for the duration of a save, delete or
    /// import. Hold the returned guard for the whole operation — DB write *and*
    /// the live-state sync that follows — so config mutations stay serialized
    /// and the in-memory engines never disagree with the database.
