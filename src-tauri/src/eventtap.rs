...
pub fn restart(app: &AppHandle) {
    // A restart swallows the release of any modifier currently held, which
    // would leave a Quick Peek stuck — dismiss it first.
...
struct ModifierEvent {
    /// Physical keycode the transition came from, used to key quick-peek
    /// begin/end bookkeeping (F18 for the Caps-Lock-as-F18 path).
    trigger_keycode: i64,
    key: ModifierKey,
    side: KeySide,
    is_down: bool,
...
/// effects: quick-peek begin/end, hyper-held tracking, the remap-stamp set
/// applied to later keystrokes, and dispatching a completed tap action.
///
...
fn drive_modifier(
    ...
    let (tap_action, remap, hyper, peek_target, held_remap_stamp) = {
        ...
        let peek_target = engine.find_rule(key, side).and_then(|r| match &r.tap {
            AppAction::LaunchApp(t) if t.quick_peek => Some(t.bundle_id.clone()),
            _ => None,
        });
        (
            action,
    ...
    // A quick-peek rule acts on the hold itself: summon on press, dismiss on
    // release. Its tap action is suppressed below, so a fast tap does not
    // also fire a second summon. The release deliberately ignores the rule
    // lookup: the rule's scope follows the frontmost app, which by now is the
    // peeked target, so re-checking would leave the peek stuck. Ending a
    // keycode that owns no peek is a no-op.
    if is_down {
        if let Some(bundle_id) = &peek_target {
            crate::peek::begin(
                app,
                crate::peek::Trigger::Modifier(trigger_keycode),
                bundle_id,
            );
        }
    } else {
        crate::peek::end(app, crate::peek::Trigger::Modifier(trigger_keycode));
    }

    if let Some(action) = tap_action {
        match action {
            AppAction::LaunchApp(t) if t.quick_peek => {}
            action => dispatch_async(app, action),
        }
    }
