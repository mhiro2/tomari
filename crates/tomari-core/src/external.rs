...
//! focused-window placement plus toggling Tomari's own panel — and is the
//! *only* way a deep link turns into an [`AppAction`]. Adding a new `AppAction`
//! variant therefore cannot accidentally expose it: a matching `ExternalAction`
//! and parser arm have to be added here on purpose.
//!
...
///
/// This is intentionally a strict subset of [`AppAction`]: focused-window
/// placement plus toggling Tomari's own panel. It bounds *what* an external
/// caller may do — not *who* may call (any local process or web page that can
/// open a URL can). Launching apps, synthesizing keystrokes, IME switching and
/// leader mode are deliberately absent.
///
/// Window placement ([`Snap`](Self::Snap), [`MoveDisplay`](Self::MoveDisplay),
/// [`Undo`](Self::Undo)) is gated behind `external_window_actions_enabled`,
/// which defaults *off* so external control is opt-in. [`TogglePanel`] is
/// exempt — see [`is_window_placement`](Self::is_window_placement).
...
impl ExternalAction {
    /// Whether this action moves the user's focused window, and so is gated
    /// behind `external_window_actions_enabled`.
    ///
    /// Everything but [`TogglePanel`](Self::TogglePanel) is window placement.
    /// `TogglePanel` only shows or hides Tomari's *own* panel — it never
    /// touches another window — and is the documented recovery route for a
    /// hidden menu bar, so it stays available even when external window
    /// control is switched off.
    pub fn is_window_placement(self) -> bool {
        matches!(self, Self::Snap(_) | Self::MoveDisplay(_) | Self::Undo)
    }
}
...
mod tests {
    ...
    #[test]
    fn only_toggle_panel_bypasses_the_window_actions_gate() {
        // Placement honors the master switch; toggling our own panel does not,
        // so it survives as the recovery route when the switch is off.
        assert!(ExternalAction::Snap(WindowPreset::LeftHalf).is_window_placement());
        assert!(ExternalAction::MoveDisplay(DisplayDirection::Next).is_window_placement());
        assert!(ExternalAction::Undo.is_window_placement());
        assert!(!ExternalAction::TogglePanel.is_window_placement());
    }
