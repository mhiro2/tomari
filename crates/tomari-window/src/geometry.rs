...
/// Smallest width/height (points) a drag-resize will shrink a window to, so the
/// window cannot be dragged down to an unusable sliver or to zero.
pub const MIN_DRAG_SIZE: f64 = 80.0;

/// The frame for a window being dragged to move: its size is unchanged and its
/// origin shifts by the cursor delta from where the drag began.
pub fn drag_move_frame(start_frame: Rect, cursor_delta: (f64, f64)) -> Rect {
    let (dx, dy) = cursor_delta;
    Rect::new(
        start_frame.x + dx,
        start_frame.y + dy,
        start_frame.width,
...
/// The frame for a window being dragged to resize from its bottom-right corner:
/// the top-left origin stays anchored and only the size grows/shrinks by the
/// cursor delta, floored at [`MIN_DRAG_SIZE`].
pub fn drag_resize_frame(start_frame: Rect, cursor_delta: (f64, f64)) -> Rect {
    let (dx, dy) = cursor_delta;
    Rect::new(
        start_frame.x,
        start_frame.y,
        (start_frame.width + dx).max(MIN_DRAG_SIZE),
        (start_frame.height + dy).max(MIN_DRAG_SIZE),
    )
}
...
mod tests {
    ...
    #[test]
    fn drag_move_shifts_origin_keeps_size() {
        let start = Rect::new(100.0, 200.0, 640.0, 480.0);
        assert_eq!(
            drag_move_frame(start, (30.0, -50.0)),
            Rect::new(130.0, 150.0, 640.0, 480.0)
        );
    }

    #[test]
    fn drag_resize_anchors_top_left() {
        let start = Rect::new(100.0, 200.0, 640.0, 480.0);
        // Origin is untouched; only the size grows by the cursor delta.
        assert_eq!(
            drag_resize_frame(start, (60.0, 120.0)),
            Rect::new(100.0, 200.0, 700.0, 600.0)
        );
    }

    #[test]
    fn drag_resize_floors_at_minimum() {
        let start = Rect::new(100.0, 200.0, 640.0, 480.0);
        // Dragging far past the top-left clamps to the minimum, never inverts.
        let resized = drag_resize_frame(start, (-5000.0, -5000.0));
        assert_eq!(
            resized,
            Rect::new(100.0, 200.0, MIN_DRAG_SIZE, MIN_DRAG_SIZE)
        );
    }
}
